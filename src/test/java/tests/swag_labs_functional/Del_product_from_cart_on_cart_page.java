...
public class Del_product_from_cart_on_cart_page extends BaseTest {

    @Test
    public void del_product_from_cart_on_cart_page() {
        ...
        /** Если продукт был добавлен в корзину, то удалим его */
        if (productIsAdd) {
            driver.findElement(By.id(String.format("remove-%s", currentProduct.toLowerCase().replaceAll(" ", "-")))).click(); // удалим товар из корзины
        } else Assert.fail("Продукт не добавился в корзину, нечего удалять");
        ...
        /** Проверим отсутствие товара который мы удалили только что */
